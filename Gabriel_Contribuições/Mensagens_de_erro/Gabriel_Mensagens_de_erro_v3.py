...
def FAIXA_IMC(IMC,magreza_grave,magreza_moderada,magreza_leve,normal,sobrepeso,obeso_I,obeso_II,obeso_III):
    '''
    >>> FAIXA_IMC(21,"magreza_grave","magreza_moderada","magreza_leve","normal","sobrepeso","obeso_I","obeso_II","obeso_III")
    ('saudável', 'normal')
    '''
    if IMC <=16:
...
def CALCULA_COMPARAÇÃO(calorias_ingeridas,RECOMENDADO,dias):
    '''
    >>> CALCULA_COMPARAÇÃO((300,200,100,100),600,2)
    'a menos que o'
    '''
...
def ganho_ou_perda_massa(calorias_ingeridas,RECOMENDADO):
    '''
    >>> ganho_ou_perda_massa((900,200,100,100),600)
    ('no ganho', 100.0)
    '''
    ganho_ou_perda=RECOMENDADO-sum(calorias_ingeridas)
    if ganho_ou_perda <=0:
