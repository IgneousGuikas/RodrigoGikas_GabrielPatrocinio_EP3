...
''' pesquisa_alimentos(catalogo, consumo_semana)
...
''' total_calorias_ingeridas(nutrientes_semana)
    
    Recebe um dicionário com as calorias e nutrientes consumidos
    pelo usuario por dia e devolve o número de dias listados e
    o total de calorias ingeridas pelo usuario
    '''
def total_calorias_ingeridas(nutrientes_semana):
    ''' Testa se total_calorias_ingeridas funciona adequadamente
        >>> total_calorias_ingeridas({'06/04/15': [296.9, 12.282, 48.77, 5.846], '07/04/15': [552.0, 33.4, 0.0, 46.4]})
        (2, 848.9)
        '''
    total = 0    
    dias = 0
    for dia in nutrientes_semana:
        total += nutrientes_semana[dia][0]
        dias += 1
    return dias, total